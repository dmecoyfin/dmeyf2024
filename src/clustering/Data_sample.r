...
# Crear nuevas columnas en el dataframe dataset
dataset <- dataset %>%
  # Crear Ratio de Activos a Pasivos
  mutate(Ratio_Activos_a_Pasivos = mactivos_margen / (mpasivos_margen + 1),
         
         # Crear Promedio de Consumos de Tarjetas
         Promedio_Consumos = (Visa_mconsumototal + Master_mconsumototal) / 
                             (Visa_cconsumos + Master_cconsumos + 1),
         
         # Crear Cambio en Rentabilidad
         Cambio_Rentabilidad = mrentabilidad_annual - (12 * mrentabilidad))

# Crear nuevas columnas en el dataframe dataset
dataset <- dataset %>%
  # Crear Ratio de Pago de Préstamos
  mutate(Ratio_Pago_Prestamos = mpago_cuotas / (msaldo_total_prestamos + 1),
         
         # Crear Promedio de Días de Mora
         Promedio_Dias_Mora = cantidad_dias_mora / (cantidad_cuotas_pagadas + 1),
         
         # Crear Proporción de Pagos Mínimos
         Proporcion_Pagos_Minimos = mpago_minimo / (mpago_cuotas + 1))
...
"Ratio_Activos_a_Pasivos", "Promedio_Consumos", "Cambio_Rentabilidad", 
"Ratio_Pago_Prestamos", "Promedio_Dias_Mora", "Proporcion_Pagos_Minimos"
)
